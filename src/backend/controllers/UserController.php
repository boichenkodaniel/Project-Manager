...
class UserController {
    ...
    public function create() {
        ...
        $required = ['login', 'password', 'fullname', 'email', 'role'];
        ...
        try {
            ...
            $this->json(['success' => true, 'message' => 'Пользователь создан', 'id' => $id], 201);
        } catch (Exception $e) {
    ...
    public function update($id) {
        ...
        $fields = ['login', 'fullname', 'email', 'role'];
        ...
        try {
            $updated = $this->model->updateUser(
                $id,
                $input['login'],
                $input['password'] ?? '', // если пароль не передан — не меняем
                $input['fullname'],
                $input['email'],
                $input['role']
            );
